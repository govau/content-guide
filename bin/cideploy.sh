...
main() {
  ...
  case "${GITBRANCH}" in
    master)
      basicauth
      mv _site/$CF_PATH/robots-production.txt _site/$CF_PATH/robots.txt
      cf api $CF_API_PROD
      cf auth $CF_USER_PROD $CF_PASSWORD_PROD
      cf target -o $CF_ORG_PROD
      cf target -s $CF_SPACE_PROD
      cf zero-downtime-push content-guide -f manifest-production.yml
      ;;
    develop)
      ...
      cf zero-downtime-push content-guide -f manifest-develop.yml
      ;;
    *)
